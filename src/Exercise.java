...
public class Exercise implements ActionListener{ 
		...
		public Exercise(String[] array, Player a, Player b){
			//created frame
			this.array = array;
			this.x = a;
			this.y = b;
			...
			okButton.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					try {
						intBest = Integer.parseInt(bestOf.getText());
					} catch(Exception ae) {}
					...
					if (intBest % 2 == 0) JOptionPane.showMessageDialog(null, "It must be an odd integer");
					else correct = true;
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (pressed == button1) {
				setText(button1, turn, 1, array, x, y);
				//switchPlayer(player);
				pressed.setEnabled(false);
			} else if (pressed == button2) {
				setText(button2, turn, 2, array, x, y);
				//switchPlayer(player);
				pressed.setEnabled(false);
			} else if (pressed == button3) {
				setText(button3, turn, 3, array, x, y);
				//switchPlayer(player);
				pressed.setEnabled(false);
			} else if (pressed == button4) {
				setText(button4, turn, 4, array, x, y);
				//switchPlayer(player);
				pressed.setEnabled(false);
			} else if (pressed == button5) {
				setText(button5, turn, 5, array, x, y);
				//switchPlayer(player);
				pressed.setEnabled(false);
			} else if (pressed == button6) {
				setText(button6, turn, 6, array, x, y);
				//switchPlayer(player);
				pressed.setEnabled(false);
			} else if (pressed == button7) {
				setText(button7, turn, 7, array, x, y);
				//switchPlayer(player);
				pressed.setEnabled(false);
			} else if (pressed == button8) {
				setText(button8, turn, 8, array, x, y);
				//switchPlayer(player);
				pressed.setEnabled(false);
			} else if (pressed == button9) {
				setText(button9, turn, 9, array, x, y);
				//switchPlayer(player);
				pressed.setEnabled(false);
			}
		}

		public void setText(JButton button, int turn, int number, String[] array, Player a, Player b) {
			boolean gotPoint = false;
			if (turn%2 == 1) {
				button.setText("O");
				array[number] = "O";
				gotPoint = checker.checkBoard(array, a);
			} else if (turn%2 == 0) {
				button.setText("X");
				array[number] = "X";
				gotPoint = checker.checkBoard(array, b);
			}
